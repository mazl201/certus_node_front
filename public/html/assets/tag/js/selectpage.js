...
;(function($){
	...
	$.extend(SelectPage.prototype,{
		...
		_setProp: function() {
			...
			this.template = {
				tag : {
					content : '<li class="selected_tag" itemvalue="#item_value#" itempid="#item_pid#" itemtext="#item_text#">#item_desc#<span class="tag_close">x</span></li>',
					textKey : '#item_text#',
					pidKey : '#item_pid#',
					valueKey : '#item_value#',
					descKey: '#item_desc#',
		...
		_addNewTag: function(self,item){
			...
			// 员工ID
			tmp = tmp.replace(self.template.tag.pidKey,item.text.split("-")[1]);
			// 员工姓名
			tmp = tmp.replace(self.template.tag.textKey,item.text.split("-")[0]);
			// 员工姓名-ID
			tmp = tmp.replace(self.template.tag.descKey,item.text);
